    ...
    return (
        <nav className="bg-white shadow-md fixed w-full z-10 top-0 left-0">
            <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
                ...
                {/* Botón Hamburguesa */}
                <button
                    className="md:hidden flex flex-col justify-between w-8 h-8 p-1 bg-gray-300 bg-opacity-30 rounded-md"
                    onClick={() => setIsOpen(!isOpen)}
                    aria-label="Toggle Menu"
                >
          <span
              className={`hamburger-bar block ${isOpen ? 'rotate-45 translate-y-2' : ''}`}
          ></span>
                    <span
                        className={`hamburger-bar block ${isOpen ? 'opacity-0' : 'opacity-100'}`}
                    ></span>
                    <span
                        className={`hamburger-bar block ${isOpen ? '-rotate-45 -translate-y-2' : ''}`}
                    ></span>
